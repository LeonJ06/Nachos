...
public class UserProcess {
     ...
     */
    public boolean execute(String name, String[] args) {	//執行
        if (!load(name, args))			//如果load(name,arge)=0 則 return false 
            return false;
        
        thread= new UThread(this);		//新建 UThread,用來處理user process
        thread.setName(name).fork();	//fork
     ...
     */		//從本Process的虛擬記憶體,傳輸資料到 所有特定的array
    ...
    public int readVirtualMemory(int vaddr, byte[] data, int offset,
        ...
        byte[] memory = Machine.processor().getMemory();	//byte型態的陣列
    ...
    private boolean load(String name, String[] args) {
        ...
        initialPC = coff.getEntryPoint();		 //初始的program counter
    ...
    protected boolean loadSections() {
        if (numPages > Machine.processor().getNumPhysPages()) {
            coff.close();
            Lib.debug(dbgProcess, "\tinsufficient physical memory");//實體記憶體不足
            return false;
        }
    ...
    if (this.process_id != ROOT) {		//在跟目錄才能執行
    	return 0;
    }
    Machine.halt();
    ...
    return 0;
         ...
         */
        private int handleExec(int file, int argc, int argv){	//檔名,argument count,argument vector
